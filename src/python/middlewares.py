...
def jsondump(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        # Keep aiohttp responses intact so their status and headers survive.
...
async def options_middleware(request, handler):
    ...
    try:
        ...
        # Preserve the real HTTP status instead of flattening errors to 200.
        if ex.status == 404:
            body = {
                "code": ex.status,
            ...
            body = {"code": ex.status, "msg": ex.reason}
        return wj(body, status=ex.status, headers=CORS_HEADERS)
    except Exception as e:
        logger.error(f"中间件处理请求时出错：{e}")
        return wj({"code": 500, "msg": str(e)}, status=500, headers=CORS_HEADERS)
