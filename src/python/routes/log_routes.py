...
async def get_realtime_logs(request):
    ...
    try:
        ...
        return wj({"code": 400, "message": "limit must be an integer"}, status=400)
    if not 1 <= limit <= 5000:
        return wj({"code": 400, "message": "limit must be 1-5000"}, status=400)
...
async def clear_logs(request):
    ...
    if request.method != "POST":
        return wj({"code": 405, "message": "Method Not Allowed"}, status=405)
    try:
        log_collector.clear()
