...
class Database:
    """数据库管理类，Bug 1 & 6 修复：添加线程锁和事务管理"""

    def __init__(self, db_path="icp_history.db"):
        self.db_path = db_path
        self._lock = threading.Lock()  # Bug 1 修复：添加线程锁
        self.init_db()

    def _get_connection(self):
        """获取数据库连接，Bug 1 修复：添加超时和线程安全"""
        conn = sqlite3.connect(self.db_path, timeout=30.0)  # 添加超时参数
    ...
    def add_history(self, search_type, search_keyword, result_count=0, result_data=None):
        ...
        with self._lock:
            try:
                ...
                cursor.execute('''
                INSERT INTO search_history (search_type, search_keyword, result_count, result_data, search_time)
                VALUES (?, ?, ?, ?, ?)
                ''', (search_type, search_keyword, result_count, result_json, now_local))
    ...
    def add_batch_task(self, task_name, task_type, total_count=0):
        ...
        with self._lock:
            try:
                ...
                now_local = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                cursor.execute('''
                INSERT INTO batch_task_history (task_name, task_type, total_count, status, create_time, update_time)
                VALUES (?, ?, ?, 'running', ?, ?)
                ''', (task_name, task_type, total_count, now_local, now_local))
