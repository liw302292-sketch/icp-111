...
impl TokenCache {
    ...
    fn max_captcha_per_token(&self) -> u32 {
        // 与 Python 配置对齐：默认 100
        100
    }
}
...
impl Beian {
    ...
    async fn get_token(
        ...
        self.get_token_inner(proxy, false).await
    }

    async fn get_token_force(
        &self,
        proxy: Option<&str>,
    ) -> Result<(String, HashMap<String, String>), String> {
        self.get_token_inner(proxy, true).await
    }
    ...
    async fn get_token_inner(
        ...
        let max_per_token;
        {
            ...
            max_per_token = cache.max_captcha_per_token();
            
            // 强制刷新 或 达到上限 或 标记为需刷新
            if force || cache.force_refresh || cache.captcha_count >= max_per_token {
                drop(cache);
                let mut cache = self.token.lock();
                if cache.captcha_count >= max_per_token {
                    info!("Token 已达 {max_per_token} 次上限，主动轮换");
                }
                cache.token.clear();
                cache.expire_ms = 0;
                cache.captcha_count = 0;
                cache.consecutive_fails = 0;
                cache.force_refresh = false;
            } else if cache.expire_ms > Self::now_ms() && !cache.token.is_empty() {
    ...
    async fn check_img(
        ...
        {
            let cache = self.token.lock();
            if cache.captcha_count >= cache.max_captcha_per_token() {
                drop(cache);
                info!("Token 已达上限，强制轮换");
                let mut cache = self.token.lock();
        ...
        if !data["success"].as_bool().unwrap_or(false) {
            ...
            {
                let mut cache = self.token.lock();
                cache.consecutive_fails += 1;
                warn!("打码失败 (连续{}次)", cache.consecutive_fails);
                if cache.consecutive_fails >= 2 {
                    warn!("连续打码失败，标记Token强制刷新");
                    cache.force_refresh = true;
                }
            }
        ...
        {
            let mut cache = self.token.lock();
            cache.captcha_count += 1;
            cache.consecutive_fails = 0;
            let max = cache.max_captcha_per_token();
            let cnt = cache.captcha_count;
            if cnt >= (max as f32 * 0.8) as u32 {
                info!("Token 使用: {cnt}/{max}，接近轮换阈值");
            }
        }
